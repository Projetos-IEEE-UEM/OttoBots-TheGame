...
if dano == false && estado!="morto" {
	...
	if (mudar_direcao == true){
		...
		move = -(keyboard_check(ord("A")) or gamepad_button_check(0, gp_padl))+(keyboard_check(ord("D")) or gamepad_button_check(0, gp_padr))
		direcao = move;
	}
}
...
if (keyboard_check(ord("S")) or gamepad_button_check(0, gp_face3)) {
	agachar = true
}
...
if !place_meeting(x, y-5, Obj_block) && !(keyboard_check(ord("S")) or gamepad_button_check(0, gp_face3)) {
	agachar = false
}

//muda a altura do pulo do player quando este esta agachado
if agachar && (keyboard_check(ord("S")) or gamepad_button_check(0, gp_face3)) {
	vel_pulo = -5
} else {
	vel_pulo = -7
}
...
if(dano == false){
	...
	if ((keyboard_check_pressed(vk_space) or gamepad_button_check_pressed(0, gp_face1)) && pulos > 0){ //Se o botão de pulo for pressionado e a condição de debounce for cumprida;
...
switch(estado){
	...
	case "pulando":{
		...
		if vel_movimento_vertical < 0 && !(keyboard_check(ord("S")) or gamepad_button_check(0, gp_face3)) {
